...
> The Godot UI domain knowledge (`godot-ui`) is not a companion skill: it ships as a plugin skill under `plugin/godot-mcp-unified/skills/godot-ui/`, surfaced natively by plugin-system clients (ZCode, Codex, DSH) and copyable from there for other clients.

## `godot-mcp-unified` — the toolkit workflow skill
