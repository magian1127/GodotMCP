# Extending the Godot MCP Unified
...
filesystem scan or when you call `discover_tools` with `refresh_extensions:true`.
...
buffer (`ws_buffer_kb`, default **1 MB**). Both channels — the editor server and
the playtest runtime server — share one transport and read the same setting, so
the ceiling is identical on either side.
...
*caller's* side (the loop an agent follows, with a worked `user_data_read` example
and workspace line-reader guidance) — your tool returning these same fields lets an
agent that already knows the built-in loop page your tool with zero new learning.
...
4. State the `godot-mcp-unified` dependency prominently in your AssetLib
...
**Local update safety:** Extensions live outside the toolkit's addon directory,
so local installer updates to `godot-mcp-unified` never touch your extension
files.

### Getting listed in the extension catalog

The dock reads the bundled local
`addons/godot_mcp_toolkit/extensions/catalog.json`. It performs no network
request and accepts only `local_path` entries under the bundled extensions
directory. A catalog entry lives in this local JSON file, not in extension code.

To get your extension listed, a maintainer needs these fields:

- **Name** — the name shown in the catalog
- **Description** — a one-line summary
- **Local path** — must stay under `res://addons/godot_mcp_toolkit/extensions`
- **Minimum toolkit version** (optional) — the earliest `godot-mcp-unified`
  your extension supports
- **Maximum toolkit version** (optional)
- **Minimum Godot version** (optional)
- **Maximum Godot version** (optional)

**How to get listed:** add the entry to the bundled local `catalog.json`, place
its files under the local extensions directory, and run the local-only policy
test. External repository fields are intentionally unsupported.

### Graceful dependency handling

Extensions depend on the Godot MCP Unified addon. Users may install your extension
...
1. State the dependency prominently in your `README.md`:
   *"Requires the Godot MCP Unified
   plugin. Install it from the Godot AssetLib (search 'Godot MCP Unified')
...
> extension is **not** applied until you restart the editor. `discover_tools`
> with `refresh_extensions:true` returns the restart `hint` in its `extension_refresh`
> summary, and a `push_warning` appears in
...
**Programmatic refresh:** As an alternative to editor focus, call
`discover_tools` with `refresh_extensions:true` to force a filesystem scan and immediate
...
When you export your game, the Godot MCP Unified must not ship in the build. The addon
...
Reusable workflows live in the bundled Codex skills rather than MCP prompts.
This keeps procedural guidance out of the always-advertised protocol surface and
lets each skill route to only the tool groups it needs. The server still exposes
read-only resources (`godot://scene/{path}`, `godot://script/{path}`,
`godot://project/info`, `godot://roots`); these providers are currently hardcoded
in the TypeScript source.

**Post-1.0 roadmap:** Resource extensibility can use the same extension-addon
pattern as tools, allowing extensions to declare resource providers alongside
tool handlers. New reusable workflows should normally be added as skills.
