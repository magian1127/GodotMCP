...
| `editor_sync` | 4.2 | Refreshes the requested `file_paths` in O(1)-per-file mode, or performs a full `scan()` when omitted, then waits for the import pipeline to become idle. Both stages work on all versions |
| `discover_tools(refresh_extensions:true)` | 4.2 | On 4.2, **editing an existing** extension is not applied in-session (a cached read avoids an engine reimport crash — see below); `extension_refresh.hint` names the extension and says to restart. Adding/removing extensions applies live. 4.3+ applies all changes live |
...
| `lsp_diagnostics`, `lsp_symbols`, `lsp_hover`, `lsp_completion`, `lsp_navigate` | 4.2 | LSP works on 4.2+. **Multi-editor conflict detection is degraded < 4.5**: the cross-project root-mismatch check needs 4.5+, so on 4.2–4.4 give each editor a distinct `--lsp-port` + `GODOT_MCP_LSP_PORT` (see [multi-instance.md](multi-instance.md)) |
| `log_read(channel:"editor")` | 4.2 | Captures runtime output (errors/warnings/prints) on all versions. **Editor parse errors** (from `editor_sync` recompiling a script) surface only on **4.5+** (the Logger API hooks editor diagnostics); on **4.2–4.4** they are NOT written to `godot.log`, so neither `source="buffer"` nor `source="file"` can return them — use `script_check` or `lsp_diagnostics` instead. Multi-line errors that ARE captured (`SCRIPT ERROR: …` + `   at: <script>.gd:LINE`) are leveled as a unit, so a filename + `level_filter:["error"]` query finds the location line. 4.5+ uses the synchronous Logger (one entry per error, zero-latency) |
...
- **Extension hot-reload — editing an existing extension needs a restart (4.2
  ...
  edit. `discover_tools(refresh_extensions:true)` returns an `extension_refresh.hint` naming the changed extension and
...
- **Live attached-script reload — editing a script bound to a running instance
  ...
  the **4.3→4.4** boundary. On 4.2 and 4.3, after workspace file editing changes a `.gd` that
  ...
  with the old behaviour and **no error**. `editor_sync`, re-`node_set_script`,
  ...
  surfaces this **reactively** on a `node_call_method` `INVALID_METHOD` whose method
  exists on the on-disk script. A
  genuine typo (method absent on disk) gets no such hint, and a compile-failed
  edit is left to `script_check` / `lsp_diagnostics`. Empirically characterised across 4.2–4.6.
...
- **GDScript LSP multi-editor — root verification needs 4.5+.** The `lsp_diagnostics`,
  `lsp_symbols`, `lsp_hover`, `lsp_completion`, and `lsp_navigate` tools
...
- **`log_read(channel:"editor")` — editor parse-error capture needs 4.5+ (4.2–4.4).** Godot's file
  ...
  `log_read` (neither `source`). Use `script_check` or `lsp_diagnostics` for parse errors
  instead. (Editor *runtime* warnings — `push_warning` from
...
- `project_delete(kind:"scene")` / `project_delete(kind:"file")`: non-active open scene tabs become phantoms
...
- Console capture uses the Logger API (zero-latency, in-memory ring buffer) instead of
  ...
  `godot.log` file logging does not, so `log_read(channel:"editor")` surfaces them here.
- `animationtree_list` enumerates state-machine nodes fully (`get_node_list` is 4.5+).

### EditorFileSystem indexing (all versions)

File-mutating MCP commands (`resource_write`, `scene_create`, `project_delete`,
etc.) call `EditorFileSystem.update_file()` and poll `get_file_type()` to confirm
indexing before returning. An `indexed` or `deindexed` field in the response
indicates whether EditorFileSystem acknowledged the change. After editing source
files through workspace file tools, call `editor_sync` for the changed paths.
...
- **SVG imports:** `asset_import` may return `class: null` if the import
  pipeline hasn't finished. Call `editor_sync` after importing.

### Phantom tab cleanup (scene/file/folder delete)

`project_delete` with `kind:"scene"`, `kind:"file"` (for `.tscn`/`.scn`), or
`kind:"folder"` attempts to close editor tabs for scenes being deleted,
preventing phantom tabs that silently recreate files on save (godot#44123).

**Godot 4.5+:** tabs are closed automatically via `close_scene_tab_safe()`.
The response includes `tab_closed: true`. For `project_delete(kind:"folder")` with exactly
...
`project_delete(kind:"folder")` uses the switch-away strategy and returns `stale_tabs` +
`warnings`.
...
**Workaround:** Use `log_read(channel:"editor", level_filter:["error"])` as a
...
| Guided onboarding wizard (3-step) | OK | OK | OK | `AcceptDialog` + `add_button()` stable since 4.0; Chinese editor locales use Chinese copy, all other locales use English |
...
| `script_check` | ✅ | |
| `folder_create` | ✅ | |
| `project_delete` | ✅ | Routes files, folders, scenes, scripts, and resources through their type-specific safety paths |
...
| `scene_instantiate` | ✅ | |
| `scene_diff` | ✅ | |
| `node_inspect` | ✅ | Single-property and filtered-list modes |
| `node_set_property` | ✅ | |
| `node_set_script` | ✅ | |
...
| `resource_load` | ✅ | |
| `resource_write` | ✅ | |
| `asset_query` | ✅ | List and dependency modes |
| `asset_import` | ✅ | |
| `user_data_read` | ✅ | File and directory modes |
| `save_write` | ✅ | |
| `save_delete` | ✅ | |
| `classdb_query` | ✅ | Search and class-info modes |
| `project_get_settings` | ✅ | |
| `project_set_setting` | ✅ | |
| `input_map_edit` | ✅ | Action and event operations |
| `animation_keyframe` | ✅ | |
| `animation_get_keys` | ✅ | |
| `tilemap_set_cells` | ✅ | |
| `scene_create_3d` / `tileset_edit` | ✅ | Editor-side resource and scene mutations |
| `editor_save_scene` | ✅ | |
| `editor_sync` | ✅ | Refreshes and waits for the import pipeline in one call |
| `log_read(channel="editor")` | ✅ | Captures available editor/runtime output; editor parse capture still varies by version |
| `game_start` | ❌ | Returns `HEADLESS_UNSUPPORTED`; use `script_check`, scene inspection, and editor-channel `log_read` |
| `game_stop` | ✅ | |
| `capture_screenshot` | ❌ | Both editor and runtime targets require a rendered viewport |
| `runtime_inspect_node` / runtime-channel `log_read` | ⚠️ | Require a game with the runtime server |
