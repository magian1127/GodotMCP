...
- `log_read(channel:"editor", source:"file")` and
  `log_read(channel:"runtime", source:"file")` read this shared log, so output
  from both instances is interleaved.
...
the most-recently-launched game process, so `debug_inspect(mode:"state")`,
`debug_continue`, and `log_read(channel:"runtime")` reflect that last instance only. For
...
`lsp_navigate` tools reach Godot's built-in **GDScript Language Server**, which binds a
...
LSP tools would reach the *first* editor.
...
> `--lsp-port` + `GODOT_MCP_LSP_PORT` before using the LSP tools with more than one
> editor open.
