@tool
extends RefCounted
## path2d.* 命令处理器 — 编辑 Path2D 的 Curve2D 点。
...
## 编辑 Path2D 场景节点上的 **Curve2D**(工具 `path2d.edit_curve`)。注意
## 这不是 `procedural.edit_curve` —— 后者创建的是 1D **Curve** .tres 资源 —
## 引擎类型不同,聚合体也不同。私有名称带有 `2d` 后缀,以便连内部
## 符号也能区分 Curve 与 Curve2D。
