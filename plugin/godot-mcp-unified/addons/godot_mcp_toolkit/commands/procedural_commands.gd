@tool
extends RefCounted
## procedural.* 命令处理器 — 创建/编辑 Gradient、Curve 与
## FastNoiseLite 资源,保存为独立的 .tres 文件。
...
## 把 1D **Curve** 资源创建为独立的 .tres 文件(工具
## `procedural.edit_curve`)。不是 `path2d.edit_curve` —— 后者编辑的是
## Path2D 场景节点上的 **Curve2D** — 引擎类型不同,聚合体也不同。
