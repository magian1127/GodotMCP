@tool
extends RefCounted
## tileset.* 结构命令:定义 TileSet 的图集源集合与层
## 骨架 — 从纹理创建、添加/移除源,以及设置各层数量。
##
## 无状态 — 每个处理器接收自己的参数 Dictionary 并返回响应
## Dictionary。持久化、层节点提示与全瓦片多边形
## 来自共享的 tileset_io.gd 叶子模块(_IO);本模块只持有
## 结构形态逻辑。瓦片集(tileset)命令组抽取出的子模块,
## 经由 `preload` 别名访问。
