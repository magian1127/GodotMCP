...
## 相对于 [param hard_max] 解析调用方可选的 classdb 页 `limit`。
## 缺省 limit 时默认为 [param hard_max](保持原有行为);高于它的值
## 会被钳制到 [param hard_max] 并标记钳制;非正值会被拒绝。
## 成功时返回 {"limit": <生效上限>, "clamped": <bool>, "error": null},
## limit 超出范围时返回 {"error": <MCPToolkitError 字典>}。
...
static func _cmd_classdb_get_info(parameters: Dictionary) -> Dictionary:
	...
	# 两阶段解析:先 ClassDB(引擎原生类),
	# 再全局类列表(用户定义的 class_name 类)。
	...
	# has_more 始终存在(完整读取时为 false)。在单节溢出时,
	# 提示构建器还会添加结构化的 next_offset 续读字段。
...
static func _cmd_classdb_search(parameters: Dictionary) -> Dictionary:
	...
	if base_class == "" and pattern == "":
		...
		var top_page := Modules.Pagination.build(
			...
			false, "", 0, "")
		# 信息性提示 —— 无论是否分页都显示,因此它在信封之后设置,
		# 而不是通过构建器由 has_more 门控的提示。
	...
	# 规范字段:total_classes + has_more 始终存在;还有更多时,构建器
	# 会在由这里组装的自然语言提示旁添加 next_offset 续读字段。
...
# -- 分节构建器 ---------------------------------------------------------------
# 每个构建器对预先取回的元数据列表(`raw`)分页,并把它的
# 节 + "total_<section>" 写入 `result`。调用方从适用的来源提供 `raw` —
# 原生类用 ClassDB.class_get_*_list(),全局(class_name)类用
# script.get_script_*_list() —— 因此构建器只依赖行数据,
# 而不依赖行是如何取回的。
...
static func _build_truncation_hint(result: Dictionary, offset: int) -> String:
	...
	if parts.is_empty():
		return ""
	if parts.size() == 1:
		# 在自然语言循环提示旁边,把续读字段作为结构化的 next_offset 呈现。
