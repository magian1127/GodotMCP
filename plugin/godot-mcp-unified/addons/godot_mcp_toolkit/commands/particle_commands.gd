@tool
extends RefCounted
## particles.* 命令处理器 — 带预设的 GPU 粒子系统创建。
...
# 用户参数名 → 材质属性前缀,用于 {prefix}_min / {prefix}_max 对。
const _RANGE_PARAMS := {
...
# 由两个数据驱动阶段共享的强制转换模式(_apply_props 阶段 7 与
# _merge_overrides 阶段 6)。每个值都复刻旧的手写梯子使用的精确转换,
# 使发出的属性值与合并后的 eff 值都保持逐字节一致:
#   INT/FLOAT/BOOL — 存储前把源值包进 int()/float()/bool()。
#   VEC3/COLOR     — 通过 _vec3()/_color() 强制转换原始用户参数(合并梯子
#                    会把 {x,y,z}/{r,g,b,a} 请求字典转换为 Vector3/Color)。
#   RAW            — 直接赋值源值(应用梯子原样存储该值;到那时
#                    预设/合并已经产生了正确的 Vector3/Color,
#                    所以按原样赋值)。
# _apply_props 使用 INT/FLOAT/BOOL/RAW(其输入已带类型);
# _merge_overrides 使用 INT/FLOAT/BOOL/VEC3/COLOR(其输入是原始用户参数)。
# VEC3/COLOR 行恰好就是应用阶段按 RAW 处理的键 ——
# 强制转换发生在合并时。
enum { _COERCE_INT, _COERCE_FLOAT, _COERCE_BOOL, _COERCE_RAW, _COERCE_VEC3, _COERCE_COLOR }

# 应用器的目标:节点还是 ParticleProcessMaterial。
enum { _TARGET_NODE, _TARGET_MATERIAL }

# "if eff.has(k): target.set(prop, cast(eff[k]));
# properties_set += 1" 这类统一应用行的数据驱动映射。
# 每行为 [eff_key, target, coerce],贡献权重 1。eff_key 总是
# 等于目标属性名,因此单个 `key` 字段身兼两职。
# 行保持应用顺序(节点组,然后材质组);真正的特例 —— 范围对
# (权重 2)、发射形状名→枚举查找、子资源、版本门控属性、
# 2D/3D 分叉 —— 不在本表中,保持显式。
# 这里的属性写入彼此独立,因此整数计数与最终对象状态
# 对它们的相对顺序不变。
const _PROP_SPEC := [
	# 节点组。
...
# 合并(把用户参数叠加到以预设为种子的 eff 字典上)中统一简单覆盖行的
# 数据驱动映射。
# 每行为 [param_key, coerce],写入 eff[param_key] = coerce(parameters[key])。
# 行顺序是承重的:每一次向 overrides_applied 的遮蔽追加都按此顺序发生,
# 且数组的元素顺序是 particles.create 响应契约的一部分 —— 因此行必须
# 严格保持这一顺序(不同于 _PROP_SPEC,其写入相互独立、顺序不变)。
# 强制转换列使用合并模式(VEC3/COLOR 会转换请求字典),而不是应用模式。
# 真正的特例 —— 范围对与发射形状覆盖 —— 不在本表中;
# 它们在 _merge_overrides 中保留专属块(形状不同:
# 一个 _min/_max 对,以及一个经名称校验的字符串),
# 并在本表之后追加。
...
# 预设以 2D 单位定义。当 type=="3d" 时会应用 _adjust_for_3d()。
const _PRESETS := {
	"fire": {
...
static func _build_gradient_texture(points: Array, interp_mode: int) -> Array:
	## 从 [{offset, color}] 创建 GradientTexture1D。
	## 返回 [GradientTexture1D, Gradient]。
...
static func _build_curve_texture(points: Array) -> Array:
	## 从 [{x, y}] 创建 CurveTexture。
	## 返回 [CurveTexture, Curve]。
	var curve := Curve.new()
	for point in points:
...
static func _apply_props(node: Node, material: ParticleProcessMaterial, eff: Dictionary) -> int:
	## 把 eff 中存在的每个统一 _PROP_SPEC 条目应用到其目标上,
	## 返回 properties_set 的增量(阶段 7 中简单属性一半的
	## 数据驱动替代)。范围对循环与发射形状
	## 名→枚举查找不在此处覆盖,保持在处理器中显式执行。
	var count := 0
	for spec in _PROP_SPEC:
		...
		match spec[2]:
			...
			_:  # _COERCE_RAW — 原样赋值(已经是正确的 Vector3/Color)。
				target.set(key, raw)
		count += 1
	return count
...
static func _merge_overrides(eff: Dictionary, parameters: Dictionary) -> Array:
	## 把请求中的用户参数叠加到以预设为种子的 eff 字典上(阶段 6),
	## 返回 overrides_applied 数组 —— 即按契约顺序排列的、遮蔽了
	## 预设值的参数名。eff 会被原地修改。三个子阶段,
	## 严格按原始顺序追加,使返回的数组逐字节一致:
	##   1. 统一的简单属性(_OVERRIDE_SPEC,按顺序);
	##   2. 范围参数(_RANGE_PARAMS → {prefix}_min/_max);
	##   3. 发射形状名称覆盖。
	## 仅当 eff 已持有该键时(范围: _min 一半),参数才遮蔽预设 ——
	## 与旧的 `if eff.has(...): overrides.append(...)` 守卫一致。
	var overrides: Array = []

	# 1. 简单用户覆盖(表驱动;原为 :464-483 的 match 梯子)。
	for spec in _OVERRIDE_SPEC:
		...
		match spec[1]:
			...
			_COERCE_COLOR:
				eff[key] = _color(raw)
			_:  # _COERCE_FLOAT — 旧梯子的默认分支。
				eff[key] = float(raw)

	# 2. 范围参数:用户的浮点数或 {min,max} → 内部的 _min/_max(原为 :485-494)。
	...
	# 3. 发射形状覆盖(原为 :496-500)。在该合并运行之前,
	# 名称已在处理器中对照 _EMISSION_SHAPES 完成校验。
...
static func _cmd_particles_create(parameters: Dictionary) -> Dictionary:
	...
	# 构建有效配置:预设为基底,用户覆盖在其上。
	var eff := preset.duplicate(true)

	# --- 合并用户覆盖(数据驱动;见 _OVERRIDE_SPEC)---
	# overrides 保存遮蔽了预设值的参数名,顺序为响应契约所期望的
	# (简单属性,然后范围参数,然后发射形状)。子资源阶段
	# 在下面向其追加;结果最后读取它。
	var overrides: Array = _merge_overrides(eff, parameters)

	# --- 应用节点 + 材质属性(数据驱动;见 _PROP_SPEC)---
	# 阶段 7 的统一单属性行由 _apply_props 应用。
	# 范围对循环与发射形状名→枚举查找不是统一的
	# (权重 2 / 值是一次枚举查找),保持在下面显式处理。这些写入
	# 彼此独立,因此计数与最终状态对顺序不变。
	properties_set += _apply_props(node, material, eff)

	# 材质上的范围参数(每个权重 2 —— 保持显式)。
	...
	# --- 子资源:color_ramp、alpha_curve、scale_curve ---
...
static func _apply_sub_resource(
	...
	## 处理 color_ramp / alpha_curve / scale_curve。返回错误字典或 null。
	var source = null
