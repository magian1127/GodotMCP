...
## 把 {r,g,b,a} 字典投影为一个 Color,通道默认值为不透明的黑色
## (绘画语义:r/g/b 默认 0.0,a 默认 1.0)。它与
## Coerce.color_from_dict(不透明白色 / tint)不同 —— 不要混淆。非字典 → 不透明黑色。
