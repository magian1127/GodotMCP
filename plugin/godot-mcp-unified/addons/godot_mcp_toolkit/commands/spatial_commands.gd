@tool
extends RefCounted
## scene.spatial_map — 当前已编辑场景的只读 2D/3D 空间布局。
##
## 弥补 LLM 空间推理盲区:每个节点的世界坐标、边界
## (2D Rect2 / 3D AABB)、尺寸,以及 LLM 无法轻易凭空推导的关系 —
## 两两重叠、包含关系、最近邻间距。
## 只读且依赖标签页(基于 EditorInterface.get_edited_scene_root 运作)。
##
## 以 `scene.*` 前缀发布(`scene.spatial_map`),因为它读取的是
## 当前编辑中的场景 —— 它应与 `scene.create`/`scene.delete` 并列,
## 而不是放在独立的 "spatial" 命名空间中。文件名词 "spatial" 只是
## 内部词汇;场景读取才是契约。

const Modules := preload("res://addons/godot_mcp_toolkit/core/modules.gd")
const Helpers = Modules.CommandHelpers

## 返回节点的硬上限(同时也约束 O(n^2) 的关系计算过程)。
...
static func _cmd_spatial_map(parameters: Dictionary) -> Dictionary:
	...
	# total_nodes:完整匹配数(超出上限也会计数);无游标 —— 用
	# 过滤器缩小范围,或提高 max_nodes 以继续。
...
## 节点的世界空间边界:Rect2(2D)或 AABB(3D);若节点没有
## 空间表示则返回 null。
...
## 用 Transform2D 把局部 Rect2 变换为世界空间 AABB(4 个角点)。
...
## 用 Transform3D 把局部 AABB 变换为世界空间 AABB(8 个角点)。
...
## 两两之间的重叠/包含/最近邻计算,仅在相同空间(2D/3D)内进行。
...
## 解析可选的区域过滤器:[x,y,w,h](2D)或 [x,y,z,sx,sy,sz](3D)。
## 返回 null(无过滤器)、Rect2、AABB 或 {error} 字典。
...
## 解析可选的半径+中心过滤器。返回 null 或 {center, radius, is_2d},
## 或 {error} 字典。
...
static func _node_is_class(node: Node, type_name: String) -> bool:
	...
	if node.is_class(type_name):
		return true
	# 自定义(class_name)脚本 —— 用脚本的 resource_path 在全局类列表中
	# 匹配。Script.get_global_name() 是 4.3+ 的,因此这里保持 4.2 兼容
	# (与 editor_helpers.class_descends_from 相同的模式)。
