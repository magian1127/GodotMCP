@tool
extends RefCounted
## audiobus.* 命令处理器 — 音频总线布局管理。
##
## 对外公布的前缀是 `audiobus.*`(音频总线路由:总线、发送、
## 效果),刻意与 `sound.*`(音频采样生成)区分开。
## 单用 "audio" 这个文件名词会把两者混为一谈 —— 按总线概念命名。
...
## 根据参数中的名称或索引解析总线。
## 若既未提供 bus_name 也未提供 bus_index,返回 -2。
## 若未找到,返回 -1。
...
static func _create_effect(type_name: String) -> AudioEffect:
	# 同时接受完整类名("AudioEffectReverb")和后缀("Reverb")。
