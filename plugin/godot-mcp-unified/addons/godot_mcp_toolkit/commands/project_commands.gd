@tool
extends RefCounted
## project.* 命令处理器 — get_settings、set_setting、层名称。
...
static func _cmd_project_set_setting(parameters: Dictionary) -> Dictionary:
	...
	# `mcp_toolkit/limits/*` 这一片段有专用写入方 —— meta.set_limits ——
	# 它会应用下限钳制。在这里拒绝整个前缀,使它保持为工具集设置
	# 的唯一许可写入路径(其余走停靠面板 UI)。
	...
	# Autoload 保护 —— 注册/注销必须经由 autoload_manage,
	# 以便编辑器的单例缓存立即刷新。
	...
	# raw_value 中的资源类型值经由 Coerce.coerce_value 的 Resource
	# 分支被 FileGuard 把关。
		...
		"register":
			...
			if not FileAccess.file_exists(script_path):
				return MCPToolkitError.fail("NOT_FOUND",
					"script not found: %s; create it with workspace file editing, then call editor_sync for that path" % script_path)
			var enabled := bool(parameters.get("enabled", true))
			var value := ("*" if enabled else "") + script_path
			# 使用 EditorPlugin API 以立即刷新编辑器缓存。
			...
			if plugin != null and enabled:
				...
				# 发出 settings_changed,让 GDScriptParser 刷新其编译
				# 缓存(add_autoload_singleton 内部会这样做;直接
				# 操作 ProjectSettings 的路径不会)。
