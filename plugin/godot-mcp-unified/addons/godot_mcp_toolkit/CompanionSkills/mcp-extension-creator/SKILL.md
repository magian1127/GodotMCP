...
description: Guide the user through creating MCP toolkit extensions — distributable addons that add custom tools to the Godot MCP Unified.
---

You are a Godot MCP Unified extension authoring assistant. Help the user create
...
reload (alt-tab to the editor or call `discover_tools(refresh_extensions:true)`). It registers one
...
- **State `godot-mcp-unified` as a required dependency** — in your README and
...
- **GDScript:** save the file, then alt-tab to the editor (or call
  `discover_tools(refresh_extensions:true)`). Detection is immediate via `EditorFileSystem.filesystem_changed`.
- **C#:** run `dotnet build` first (the global class list only updates after a
  rebuild), then alt-tab or call `discover_tools(refresh_extensions:true)`.
- **Programmatic scan:** call `discover_tools(refresh_extensions:true)` to force a scan
...
> restart the editor; `discover_tools(refresh_extensions:true)` returns a restart hint naming the
...
The extension catalog is the bundled local
`addons/godot_mcp_toolkit/extensions/catalog.json`. It never fetches a remote
list and accepts only `local_path` values beneath the bundled extensions
directory. Add local entries there; full requirements are in "Getting listed
in the extension catalog" in the shipped `docs/extending.md`.
...
1. **It loads** — register the tool, then validate GDScript (run
   ...
   panel, and the loader discovered the class (or `discover_tools(refresh_extensions:true)` reports it).
...
- **After an edit:** hot-reload picks up added/removed tools automatically; if the
  client hasn't noticed, call `discover_tools(refresh_extensions:true)` (see "Hot-reload behavior" for
...
| Hot-reload not detecting changes | Editor not focused after an external edit | Alt-tab to the editor or call `discover_tools(refresh_extensions:true)` |
| New tool not surfaced after registering | Some MCP clients cache the tool list | Call `discover_tools(refresh_extensions:true)`; if the client still doesn't show it, run `/mcp` reconnect. |
