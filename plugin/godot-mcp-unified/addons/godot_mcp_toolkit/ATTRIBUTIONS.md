...
## Upstream artwork exclusion

The upstream project logo and hero banner were created by **Jessica Mariana
Aisen** and are not licensed for reuse. Those brand assets are not distributed
by Godot MCP Unified. The current geometric MCP icon was created locally for
this repository and does not reuse the upstream artwork.
...
Source: <https://github.com/Delsin-Yu/GDEditorBridge>
License: MIT

Copied into: `probe/McpToolkitAlcProbe.cs` (adapted — renamed producer, marker
paths and log prefixes; the AlcId/ISerializationListener marker mechanism is a
near-verbatim port of `GdEditorBridgeAlcProbe.cs`).

Contributed (architecture reference — reimplemented, not copied): the C# build
success semantics (exit-0 skepticism, engine-load-path DLL check, mtime
"unchanged" outcome), the ALC seven-state verdict machine with millisecond
marker comparison and the same-ALC genuine-failure rule
(`commands/editor/editor_csharp_build.gd`), the per-type button-press dispatch
required on 4.7.x because `BaseButton.press()` is not script-exposed
(`commands/editor/editor_dialogs.gd`), the editor extra-window enumeration
shape, and the reverse-dependency inverted-index idea
