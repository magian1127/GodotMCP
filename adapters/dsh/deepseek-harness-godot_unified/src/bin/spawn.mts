// Windows 命令解析与跨平台 spawn(沿智谱三件套已验证策略)。
//
// 不用 shell: true 拼接用户参数;PowerShell 的 .ps1 优先级、npm/pnpm
// Node .cmd shim 直连解析;无法安全转发的参数必须失败而非静默篡改。
import { spawnSync } from 'node:child_process'
import { existsSync, readFileSync } from 'node:fs'
import { basename, delimiter, dirname, extname, isAbsolute, join, resolve } from 'node:path'
import { WINDOWS_COMMAND_ENCODED, WINDOWS_COMMAND_ENV, type SpawnOptions, type SpawnResult } from './cli/constants.mjs'
...
/** Windows 优先直接执行 .exe 或解析 Node .cmd shim;无法解析时经编码 PowerShell 转发。 */
